...
for file_input in ../data/pos_*_in.txt; do
    ...
    in1_file="$(basename "$file_input" in.txt)in_file1.txt"
    in2_file="$(basename "$file_input" in.txt)in_file2.txt"
    file_args="$(basename "$file_input" in.txt)args.txt"
    out_file="$(basename "$file_input" in.txt)out_file.txt"
    out_file_res="$(basename "$file_input" in.txt)out_file.txt"

    ./pos_case.sh "$in1_file" "$in2_file" "$file_args" "$out_file" "$out_file_res"
    result=$?
